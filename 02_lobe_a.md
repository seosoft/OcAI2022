...
学習させる画像を取り込みます。  
Lobeには、Webカメラの画像を連続して取り込んですぐに学習させる機能があります。
...
4. 同じ要領でラベルを "Paper" にして、じゃんけんのパーを連写します。  
...
5. 同じ要領でラベルを "None" にして、**手が写らないように** 連写します。  
   これはじゃんけんの手が写っていない画像を追加することで **何もない (じゃんけんではない)** ことを学習させるためのものです。

   > "None" ラベルはなくてもいいこともありますが、それ以外のラベルの精度を上げるために役に立つことがよくあります。  
   > データを用意できるのであれば "None" を追加するのがいいでしょう。
...
2. 学習を最適化してスコアを上げてみます。  
   ...
   > 学習の最適化とは画像を学習しなおして品質の高いモデルを作成することです。その分、学習に時間がかかります。
...
なお、このステップでモデルを作った場合は [事前に用意した画像ファイルを使用](./02_lobe_b.md) して学習する手順については演習する必要はありません。  
